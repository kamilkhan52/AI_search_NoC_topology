...
def iterate(n):
    ...
    for i in range(1, n):
        ...
        if objective <= previous_objective:
            ...
            new_grid = grid.copy()
            print("-Objective: " + str(previous_objective))
...
# plt.show()
print("Final local traffic weighted hop count = " + str(previous_objective))
print("Final local app mapping = " + str(app_mapping))
...
get_initial_topology()
# perturb and predict
for g in range(1, num_iterations):
    for s in range(1, num_iterations * 3):
        get_initial_topology()
        perturb(5)
        ...
        if previous_prediction == 0:
            ...
            grid = new_grid.copy()
            starting_point = new_grid.copy()

        if prediction < previous_prediction:
            # accept perturbation
            print("↓Prediction: " + str(round(objective, 2)))
            grid = new_grid.copy()
            app_mapping = app_mapping_suggested.copy()
            traffic_data = traffic_data_suggested.copy()
            previous_prediction = prediction
    ...
    print("Running Local Search with new starting point with expected solution: " + str(round(prediction, 2)))

    grid = starting_point.copy()
    firstTime = True
    reset_temperature()
    app_mapping = np.arange(0, 64)
    app_mapping_suggested = app_mapping
...
draw_topology(grid)
print("Final cost: " + str(objective))
